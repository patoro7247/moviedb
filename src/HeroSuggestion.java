...
@WebServlet(name = "HeroSuggestion", urlPatterns = "/hero-suggestion")
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			try (Connection conn = dataSource.getConnection()) {
				...
				String fulltextParameter ="";
				if(query.length() > 0){
					fulltextParameter = '+'+query;
				...
				statement.setString(1, fulltextParameter);
			...
			for (Integer id : superHeroMap.keySet()) {
				...
				if (heroName.toLowerCase().contains(query.toLowerCase())) {
					jsonArray.add(generateJsonObject(id.toString(), heroName));
				}
			}
